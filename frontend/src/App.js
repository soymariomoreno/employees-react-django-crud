...
class App extends Component {
  ...
  renderItems = () => {
    ...
    return newItems.map(item => (
      <tr key={item.id}>
        ...
        <td >{item.about}</td>
        <td>
          <a href={item.avatar} target="blank" className="btn btn-outline-primary mb-1">
        ...
        <td>
          <button onClick={() => this.editItem(item)} className="btn btn-success mb-1">
            {" "}
  ...
  handleSubmit = item => {
    ...
    if (item.id) {
      axios
        .put(`http://127.0.0.1:8000/api/employees/${item.id}/`, form_data, {
          headers: {
    ...
    axios
      .post("http://127.0.0.1:8000/api/employees/", form_data, {
        headers: {
          'content-type': 'multipart/form-data'
        }
      })
